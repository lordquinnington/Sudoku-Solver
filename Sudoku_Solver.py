...
def findPossibleGrids(gridArray,squareSize):    # function which generates copies of the grid with a certain square guessed at
            ...
            for k in range(squareSize):
                for l in range(squareSize):
                    if gridArray[i][j][k][l][0] == 0:
                        if (len(gridArray[i][j][k][l])-1) < numberOfPossibleGrids:     # loops through the grid to find the position with the least amount of possibilities
                            numberOfPossibleGrids = len(gridArray[i][j][k][l])
                            leastPossibleOptionsPos = [i,j,k,l]
    if numberOfPossibleGrids == 9:     # if the possibilities haven't been narrowed down, the grid is deemed too hard to solve
        return None, None
    a = leastPossibleOptionsPos[0]    # could technically lose these 4 lines but it makes it easier to see
    b = leastPossibleOptionsPos[1]
    c = leastPossibleOptionsPos[2]
    d = leastPossibleOptionsPos[3]
    possibleGrids = []
    for i in range(numberOfPossibleGrids-1):    # loops through and creates seprate grids for as many possibilities as there are
...
def solve(gridArray,squareSize):    # function to solve the grid
    ...
    while not finished:
        notChanged = False
        while not notChanged:     # loops round until the grid doesn't change, indicating it has got stuck
        ...
        if possibleGrids == None:
            return None
        for i in range(numberOfPossibleGrids):    # loops through and attempts to solve each combination of grid until one returns as solved or they all don't work
            gridAttempt = copy.deepcopy(possibleGrids[i])
            gridAttempt = solve(gridAttempt,squareSize)
            if gridAttempt != None:
                return gridAttempt
        return None    # if none of them come back as solved, the grid is deemed unsolvable
    return gridArray
